...
export type WebsiteCardBorderHover = "css-vars" | "js";

function getCardHoverClass(mode: WebsiteCardBorderHover): string {
  if (mode === "css-vars") return "card-dynamic-border hover:-translate-y-1";
  return "hover:scale-[1.02] hover:-translate-y-1";
}

interface WebsiteCardProps {
  website: WebsiteDataType;
  borderHoverMode?: WebsiteCardBorderHover;
}
...
export function WebsiteCard({
  ...
  const linkStyle =
    !isComingSoon && borderHoverMode === "css-vars"
      ? ({ ["--card-border"]: colorBorder, ["--card-border-hover"]: colorBorderHover } as React.CSSProperties)
      : undefined;
  ...
  return (
    <Link
      ...
      style={linkStyle}
      onMouseEnter={
        borderHoverMode === "js" && !isComingSoon
          ? (e) => {
    ...
    >
      <div
        className={cn(
          ...
          isComingSoon
            ...
            : getCardHoverClass(borderHoverMode)
        )}
        style={{
      ...
      >
        ...
        <div className="absolute bottom-0 right-0 w-20 h-20 bg-gradient-to-tl from-white/10 to-transparent rounded-tl-full opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
      </div>
    </Link>
  );
}
