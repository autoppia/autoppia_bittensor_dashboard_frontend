// src/app/shared/table/score/columns.tsx
"use client";
...
export function buildMinerScoreColumns(data: TableDataType[]) {
  if (!data?.length) return [];

  /* 1) Todos los validators presentes en el dataset */
  const validatorIds = Array.from(
    new Set(
      data.flatMap((r) =>
        Object.keys(r.scores_per_validator ?? {}).map((k) =>
  ...
  const dynamic = validatorIds.map((id) =>
    column.accessor((row) => row.scores_per_validator?.[`validator_${id}`], {
      id: `validator_${id}`,
  ...
  const avg = column.accessor("score_avg", {
    id: "score_avg",
    header: "Average Score",
    size: 80,
    enableSorting: true,
    cell: ({ getValue }) => (getValue() != null ? getValue()!.toFixed(3) : "-"),
  });
