...
export default function AgentValidators({ selectedRound }: { selectedRound?: number | null }) {
  ...
  const avgScore =
    scoreValues.length > 0
      ? ((scoreValues.reduce((sum, value) => sum + value, 0) / scoreValues.length) * 100).toFixed(1)
      : "0";
  ...
  const tasksValues = filteredRuns
    .map((run) =>
      typeof run.completedTasks === "number" && Number.isFinite(run.completedTasks) ? run.completedTasks : null
    )
    .filter((value): value is number => value !== null);
  const avgTasks =
    tasksValues.length > 0
      ? Math.round(tasksValues.reduce((sum, value) => sum + value, 0) / tasksValues.length).toString()
      : "0";
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-gray-800 border border-gray-600 rounded-xl p-4 shadow-sm hover:bg-gray-700 transition-colors duration-200" style={{ backgroundColor: '#1f2937' }}>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h4 className="font-semibold text-white" style={{ color: '#ffffff' }}>Response Time</h4>
                </div>
                <p className="text-sm text-gray-300 leading-relaxed" style={{ color: '#d1d5db' }}>
