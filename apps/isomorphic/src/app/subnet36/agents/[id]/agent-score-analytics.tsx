...
interface AgentScoreAnalyticsProps {
  className?: string;
...
export default function AgentScoreAnalytics({
  ...
  agent,
  roundMetrics,
}: AgentScoreAnalyticsProps) {
  if (!agent) {
    return <AgentScoreAnalyticsPlaceholder className={className} />;
  }

  const currentScore = ((roundMetrics?.score ?? agent.currentScore ?? 0) * 100).toFixed(1);
  const topScore = ((roundMetrics?.topScore ?? agent.currentTopScore ?? 0) * 100).toFixed(1);

  const cards = [
    {
      title: "Current Score",
      metric: `${currentScore}%`,
      description: "Average evaluation score in the selected round",
      icon: LuStar,
      className:
      ...
      iconClassName:
        "relative bg-gradient-to-br from-emerald-400 via-emerald-500 to-emerald-600 text-white shadow-lg group-hover:scale-105 group-hover:rotate-2 transition-all duration-500 before:absolute before:inset-0 before:bg-gradient-to-br before:from-white/20 before:to-transparent before:rounded-xl",
      descriptionClassName: "text-emerald-100/80",
    },
    {
      title: "Round Top Score",
      metric: `${topScore}%`,
      description: "Highest score achieved across agents in the selected round",
      icon: LuCrown,
      className:
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full justify-center items-center text-center">
                  ...
                  <div className="flex-1 flex flex-col justify-center items-center text-center px-2">
                    ...
                    <Text
                      className={cn(
                        "text-xs font-medium leading-relaxed text-center",
