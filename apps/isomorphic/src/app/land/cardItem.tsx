...
export function WebsiteItem({ website }: WebsiteItemProps) {
  return (
    <Link
      ...
      target="_blank"
      className={cn(
        "group relative flex flex-col h-full overflow-hidden rounded-lg border-2 border-cyan-500/30 bg-background/50 backdrop-blur-md",
        ...
        <div className="relative aspect-video w-full overflow-hidden">
          ...
          {!website.isComingSoon && (
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
              <div className="flex items-center gap-2 px-6 py-3 rounded-lg border-2 border-cyan-400 bg-cyan-500/20 backdrop-blur-md text-cyan-300 font-bold transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
